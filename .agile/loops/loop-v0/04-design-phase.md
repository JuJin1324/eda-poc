...
| Phase | 상태 | 근거 |
|------|------|------|
| **Phase 1** | ✅ 완료 | `sprint-0` 완료 (실제 완료일: 2026-03-02) |
| **Phase 2~5** | ⏳ 예정 | `sprint-1` 이후 순차 진행 |

### Phase 1: EDA 핵심 개념 학습 (02/28 ~ 03/03, 4일) ✅ 완료
...
**Phase 1 상태:** ✅ 완료 (`sprint-0` 완료 기준, 실제 완료일 2026-03-02)

> 이 Phase의 학습 문서(`docs/concepts/`)와 US 회고(`.agile/`)가
> Phase 5 포트폴리오의 "AI 사고증폭기 활용 증거"로 직접 연결된다.

### Phase 2: 기반 구축 + 기본 이벤트 흐름 (03/04 ~ 03/08, 5일)
...
### Phase 4: EDA 심화 개념 학습 + 안티패턴/BP 조사 + 실무 도입 전략 수립 (03/15 ~ 03/20, 6일)

**목표:** 구현하지 않은 심화 개념 학습 + EDA 안티패턴/베스트 프랙티스 조사 + PoC·조사 결과를 종합한 실무 도입 전략 문서 작성
...
| **[안티패턴]** Kafka-as-DB | Consumer가 과거 메시지를 항상 full-scan하는 잘못된 사용 패턴 |
| **[안티패턴]** Chatty Topics | 너무 잘게 쪼갠 토픽 → 관리 비용 폭증 |
| **[안티패턴]** 멀티스레드 파티션 소비 | Consumer 하나가 단일 파티션을 여러 스레드로 처리 → 순서 보장 파괴 |
| **[안티패턴]** DLQ 없는 무한 재시도 | 처리 실패 시 재시도만 반복 → 파이프라인 정지 |
| **[안티패턴]** 이벤트 과적 | 이벤트에 너무 많은 데이터 포함 → 토픽이 데이터 저장소로 변질 |
| **[BP]** Fact 중심 이벤트 설계 | 이벤트는 사실(Fact) 중심 — 명령(Command)과 혼용 금지 |
| **[BP]** Consumer Group 격리 | 도메인별 독립 소비 — 하나의 Group에 여러 목적 혼재 금지 |
| **[BP]** Schema 버전 관리 | 하위 호환 유지 전략 (Backward/Forward Compatibility) |
| **[BP]** 모니터링 필수 항목 | Consumer Lag, 메시지 처리율, DLQ 메시지 수 |
| **실무 도입 전략 문서 작성** | PoC 경험 + 사례 연구 + 안티패턴/BP를 종합한 의사결정 가이드 (`docs/practical/eda-adoption-strategy.md`) |

**Phase 4 완료 기준:**
- 심화 개념 각각을 "무엇이고, 왜 이번에 안 썼고, 언제 필요한지" 설명 가능
- 안티패턴/BP 조사 완료 및 문서화
- 실무 도입 전략 문서(`docs/practical/eda-adoption-strategy.md`) 작성 완료 (기술 선택 + 설정값 의사결정 가이드 포함)
...
- 안티패턴/BP 조사 → "EDA 도입 시 주의할 점은?" 질문에 근거 있는 답변 가능
- 실무 도입 전략 문서 → Phase 5 트레이드오프 문서의 직접적인 근거 자료로 활용

### Phase 5: 포트폴리오 완성 (03/21 ~ 03/24, 4일)
...
| 포트폴리오 스토리라인 설계 | 동시성 제어 PoC → EDA PoC 연결 서사, 면접 예상 질문별 답변 포인트 정리<br>**(추가)** AI 사고증폭기 활용 서사 — 학습→이해→구현의 의도적 순서, 단순 코드 생성과의 차별점 |
| **문서 작성** | |
| README 작성 | 가상 시나리오 + 아키텍처 + 실행 방법 + 성능 비교 결과 |
| 성능 비교 리포트 | TPS/Latency 그래프, Before/After 수치 비교 |
| 트레이드오프 문서 | "왜 Exactly Once를 안 썼는가", "왜 Spring Cloud가 필요 없는가" 등 (Phase 4 학습 반영, `docs/practical/eda-adoption-strategy.md`를 의사결정 근거로 연결) |
| 다음 Step 로드맵 | 이번에 안 한 것들의 도입 시나리오 정리 (Phase 4 인사이트 반영) |
| **AI 활용 방법론 문서 (신규)** | README "접근 방법" 섹션 또는 별도 문서로:<br>- Phase 1 학습을 먼저 한 이유<br>- US 회고 문서를 학습 증거로 연결<br>- how-diagram.md 설계 변화(Fan-out 재정의)가 학습의 산물임을 서술<br>- "AI = 코드 자동화 도구"가 아닌 "AI = 사고를 더 날카롭게 만드는 도구" |

**Phase 5 완료 기준:**
- 문서 설계(구조/스토리라인) 완성 + README만 보고 프로젝트 실행 + 성능 비교 가능
- 면접에서 "왜 이렇게 했는가" + "왜 이건 안 했는가" 모두 설명 가능
- 포트폴리오를 본 면접관이 "AI를 어떻게 썼나요?"에 대해 스스로 답을 찾을 수 있을 정도로 학습-구현 과정이 드러나는가
